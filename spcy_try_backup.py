...
def spacy_match(resumes, jd, role):
    ...
    #jd_sum = summarizer(jd_text, max_length=100, min_length=30, do_sample=False).replace('•', '')
    ...
    for resume in resumes:
        ...
        if len(final_common_skills) + len(final_not_common_skills) > 0:
            final_percentage = (((len(final_common_skills) * 100) / (len(final_common_skills) + len(final_not_common_skills))) + match_percentage)/2
        else:
            final_percentage = match_percentage
